"""MathTutor-RL rollout function for the SLIME training loop.

Follows the same pattern as openclaw_rollout.py / openclaw_opd_rollout.py,
wiring the MathTutorAPIServer into the async rollout worker and draining
completed training samples each time the trainer requests a batch.
"""
...
from math_tutor_api_server import MathTutorAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = MathTutorAPIServer(
            args=args,
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[MathTutorWorker] waiting for samples: "
                f"{len(data)}/{target_data_size}, "
    ...
    print(
        f"[MathTutorWorker] drained {len(data)} groups in {time.time() - start:.2f}s",
        flush=True,
    )
    return data


def generate_rollout_math_tutor(args, rollout_id, data_buffer, evaluation=False):
    """Entry point registered via --rollout-function-path."""
    ...
    if eval_scores:
        ...
        extra_metrics = {"rollout/sympy_reward": avg}
        print(
            f"[MathTutorWorker] sympy_reward={avg:.4f} (n={len(eval_scores)})",
            flush=True,
        )
